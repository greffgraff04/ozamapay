          ...
          <motion.div variants={itemVariants} className="space-y-8">
            ...
            <motion.div variants={itemVariants} className="space-y-4">
              ...
              <h1 className="text-6xl sm:text-7xl lg:text-8xl font-black tracking-tight leading-[1.05]">
                <span className="block text-white">LAJAN W</span>
            ...
            {/* Stats */}
            <motion.div variants={itemVariants} className="grid grid-cols-3 gap-8 pt-10 border-t border-slate-800/60">
              {stats.map((stat, index) => (
                <div key={index} className="space-y-1">
                  <p className="text-3xl font-black text-orange-500">{stat.label}</p>
                  <p className="text-sm text-slate-500 font-bold uppercase tracking-wider">{stat.value}</p>
                </div>
              ))}
            </motion.div>
          </motion.div>
