...
export class AdminService {
  ...
  // A DailyAccessCode row with code === PAUSE_MARKER is not a real code — it's
  // a placeholder recording "requirement suspended until expiresAt" so a pause
  // survives process restarts and the midnight cron below doesn't clobber it.
  private static readonly PAUSE_MARKER = 'PAUSED';

  private async getActivePause() {
    const latest = await this.prisma.dailyAccessCode.findFirst({ orderBy: { generatedAt: 'desc' } });
    if (latest && latest.code === AdminService.PAUSE_MARKER && latest.expiresAt > new Date()) return latest;
    return null;
  }

  @Cron('0 4 * * *', { timeZone: 'America/Port-au-Prince' }) // 00:00 Haiti time (AST UTC-4 = 04:00 UTC)
  async autoGenerateDailyCode(): Promise<void> {
    if (await this.getActivePause()) return; // requirement manually suspended — don't override
  ...
  async pauseDailyCodeRequirement(hours: number): Promise<{ resumeAt: Date }> {
    await this.prisma.dailyAccessCode.updateMany({ where: { isActive: true }, data: { isActive: false } });

    const resumeAt = new Date(Date.now() + hours * 60 * 60 * 1000);
    await this.prisma.dailyAccessCode.create({
      data: { code: AdminService.PAUSE_MARKER, expiresAt: resumeAt, isActive: false },
    });

    await this.prisma.adminActionLog.create({
      data: {
        adminId: MASTER_ID,
        action: 'DAILY_CODE_REQUIREMENT_PAUSED',
        targetType: 'DailyAccessCode',
        details: `Egzijans kòd jounalye sispann pou ${hours}h (rezoud pwoblèm livrezon Brevo) — reprann otomatikman ${resumeAt.toISOString()}`,
      },
    });
    return { resumeAt };
  }

  // Verifye toutle 15 min si yon poz fin ekspire pou reyaktive egzijans lan
  // otomatikman — san sa, poz la ta ka rete pou tout tan si pesonn pa panse
  // pou remete l aktif alamen.
  @Cron('*/15 * * * *')
  async resumeDailyCodeAfterPause(): Promise<void> {
    const latest = await this.prisma.dailyAccessCode.findFirst({ orderBy: { generatedAt: 'desc' } });
    if (!latest || latest.code !== AdminService.PAUSE_MARKER || latest.expiresAt > new Date()) return;
    ...
    await this.prisma.adminActionLog.create({
      data: {
        adminId: MASTER_ID,
        action: 'DAILY_CODE_REQUIREMENT_AUTO_RESUMED',
        targetType: 'DailyAccessCode',
        details: 'Poz 24h fin ekspire — egzijans kòd jounalye reyaktive otomatikman',
      },
    });
  }
